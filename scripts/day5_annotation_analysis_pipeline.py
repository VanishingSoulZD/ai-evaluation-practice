...
DEFAULT_RULES: dict[str, list[str]] = {
    "PER": [r"王\S{1}", r"李\S{1}", r"刘\S{1}", r"张\S{1}", r"赵\S{1}", r"陈\S{1}", r"小张", r"小王", r"小李"],
    "ORG": [r"腾讯", r"华为", r"OpenAI", r"阿里巴巴", r"字节跳动", r"北京大学", r"清华大学", r"复旦大学", r"微软亚洲研究院"],
    "LOC": [r"北京", r"上海", r"深圳", r"广州", r"杭州", r"成都", r"南京", r"武汉", r"香港", r"旧金山", r"台北"],
    "TIME": [r"今天", r"昨天", r"明天", r"周[一二三四五六日天]", r"下周\S*", r"本周\S*", r"\d{4}年\d{1,2}月\d{1,2}日"],
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("reports/day5_pipeline"), help="输出目录")
    parser.add_argument("--rules-file", type=Path, default=None, help="实体抽取规则文件（JSON）")
...
def annotate_missing(df: pd.DataFrame, task_col: str, text_col: str, label_col: str, issue_col: str, rules: dict[str, list[str]]) -> pd.DataFrame:
    out = df.copy()
    ...
    def build_label(row: pd.Series) -> tuple[str, str]:
        ...
        return ("; ".join(parts) if parts else "NEU", "自动补标：原标注缺失，依据 Day1-2 规则启发式填充。")
...
def extract_sentiment_counts(df: pd.DataFrame, label_col: str) -> pd.Series:
    ...
    return pd.Series({lb: labels.str.contains(rf"\b{lb}\b").sum() for lb in SENTIMENT_ORDER}, dtype="int64")


def build_summary(df: pd.DataFrame, id_col: str, text_col: str, label_col: str) -> pd.DataFrame:
    ...
    rows = [
        ...
        {"metric": "duplicate_id_rows", "value": int(df.duplicated(subset=[id_col]).sum()) if id_col in df.columns else 0},
        {"metric": "duplicate_text_rows", "value": int(df.duplicated(subset=[text_col]).sum()) if text_col in df.columns else 0},
    ]
    rows += [{"metric": f"sentiment_{k}", "value": int(v)} for k, v in sentiment_counts.items()]
    rows += [{"metric": f"entity_{k}", "value": int(v)} for k, v in entity_counts.items()]
...
def main() -> None:
    ...
    annotated = annotate_missing(df, args.task_col, args.text_col, args.label_col, args.issue_col, rules)
    ...
    log_df = annotated[[c for c in [args.id_col, args.text_col, args.label_col, args.issue_col] if c in annotated.columns]].copy()
    log_df.to_csv(args.output_dir / "annotation_log.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(args.output_dir / "summary_stats.csv", index=False, encoding="utf-8-sig")
    sentiment_counts.rename_axis("label").reset_index(name="count").to_csv(args.output_dir / "sentiment_distribution.csv", index=False, encoding="utf-8-sig")
    entity_counts.rename_axis("label").reset_index(name="count").to_csv(args.output_dir / "entity_distribution.csv", index=False, encoding="utf-8-sig")
    bio_matrix.to_csv(args.output_dir / "bio_distribution.csv", encoding="utf-8-sig")

    if args.excel:
        with pd.ExcelWriter(args.output_dir / "day5_stats.xlsx", engine="openpyxl") as writer:
    ...
    print(f"Day 5 流程完成：已处理 {len(annotated)} 条样本，输出目录：{args.output_dir}")
