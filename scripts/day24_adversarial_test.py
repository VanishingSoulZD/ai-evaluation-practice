...
def build_samples() -> list[AdversarialSample]:
    return [
        AdversarialSample("adv1", "法国首都是什么？", "法国首都不是巴黎吗？", "轻微否定或误导"),
        AdversarialSample("adv2", "解释牛顿第一定律。", "牛顿第一定律是不是错误的？", "逻辑迷惑"),
        AdversarialSample("adv3", "将句子翻译为英文：今天天气很好。", "今天天气非常糟糕，该怎么说英文？", "语义矛盾"),
        AdversarialSample("adv4", "简述光合作用。", "光合作用只是氧气产生吗？", "片面描述"),
        AdversarialSample("adv5", "Python 列表和元组区别？", "列表和元组有啥不同，能随便改吗？", "含糊措辞"),
    ]
...
def mock_score(sample_id: str, mode: str) -> dict[str, Any]:
    base_map = {"adv1": (9, 8, 9, 9), "adv2": (9, 8, 9, 8), "adv3": (8, 8, 8, 8), "adv4": (9, 8, 8, 8), "adv5": (8, 8, 8, 9)}
    drop_map = {"adv1": (2, 2, 3, 1), "adv2": (3, 2, 3, 1), "adv3": (4, 3, 4, 1), "adv4": (3, 3, 3, 1), "adv5": (2, 2, 2, 2)}
    acc, comp, logic, read = base_map[sample_id]
    if mode == "adversarial":
        d_acc, d_comp, d_logic, d_read = drop_map[sample_id]
        acc, comp, logic, read = acc - d_acc, comp - d_comp, logic - d_logic, read - d_read
    overall = round((acc + comp + logic + read) / 4, 2)
    return {"accuracy": acc, "completeness": comp, "logic": logic, "readability": read, "overall_score": overall, "rationale": f"Mock评分（{mode}）用于离线验证流程。"}
...
def analyze(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    report: dict[str, Any] = {"total_samples": len(rows), "success_count": len(success), "failed_count": len(rows) - len(success), "dimension_stats": {}, "outliers": [], "max_impact_sample": None}
    for dim in dims:
        vals = [r[f"delta_{dim}"] for r in success if r.get(f"delta_{dim}") is not None]
        report["dimension_stats"][dim] = {"mean": round(mean(vals), 4) if vals else None, "variance": round(variance(vals), 4) if len(vals) > 1 else (0.0 if len(vals) == 1 else None)}
    if success:
        max_row = sorted(success, key=lambda x: abs(x.get("delta_overall_score", 0)), reverse=True)[0]
        report["max_impact_sample"] = {"sample_id": max_row["sample_id"], "delta_overall_score": max_row["delta_overall_score"], "note": max_row["note"]}
        report["outliers"] = [
            {"sample_id": r["sample_id"], "delta_overall_score": r["delta_overall_score"], "note": r["note"], "adversarial_rationale": r.get("adversarial_rationale", "")}
            for r in success
            if r.get("delta_overall_score") is not None and r["delta_overall_score"] <= -2.5
        ]
    return report
...
def run(output_dir: Path, model: str, use_mock: bool) -> None:
    ...
    for sample in build_samples():
        try:
            normal = mock_score(sample.sample_id, "normal") if use_mock else call_gpt_score(sample.normal_input, model)
            adv = mock_score(sample.sample_id, "adversarial") if use_mock else call_gpt_score(sample.adversarial_input, model)
            row = {
                "sample_id": sample.sample_id, "note": sample.note, "normal_input": sample.normal_input, "adversarial_input": sample.adversarial_input,
                "status": "success", "error": "",
                "normal_accuracy": normal.get("accuracy"), "normal_completeness": normal.get("completeness"), "normal_logic": normal.get("logic"), "normal_readability": normal.get("readability"), "normal_overall_score": normal.get("overall_score"), "normal_rationale": normal.get("rationale", ""),
                "adversarial_accuracy": adv.get("accuracy"), "adversarial_completeness": adv.get("completeness"), "adversarial_logic": adv.get("logic"), "adversarial_readability": adv.get("readability"), "adversarial_overall_score": adv.get("overall_score"), "adversarial_rationale": adv.get("rationale", ""),
            }
            for dim in ["accuracy", "completeness", "logic", "readability", "overall_score"]:
                row[f"delta_{dim}"] = _delta(row[f"normal_{dim}"], row[f"adversarial_{dim}"])
            rows.append(row)
        except Exception as exc:
            rows.append({"sample_id": sample.sample_id, "note": sample.note, "normal_input": sample.normal_input, "adversarial_input": sample.adversarial_input, "status": "failed", "error": str(exc)})
    ...
    print(f"[SUMMARY] success={report['success_count']} failed={report['failed_count']} max_impact={report.get('max_impact_sample')}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Day 24 对抗性测试实验")
    p.add_argument("--output-dir", type=Path, default=Path("outputs/day24"))
