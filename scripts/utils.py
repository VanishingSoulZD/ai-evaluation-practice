...
def extract_bio_matrix(df: pd.DataFrame, label_col: str) -> pd.DataFrame:
    bio_counter: dict[str, Counter[str]] = {
        prefix: Counter() for prefix in ["B", "I", "O"]
    }
    for text in df[label_col].fillna("").astype(str):
    ...
    all_types = sorted({etype for c in bio_counter.values() for etype in c.keys()}) or [
        "NONE"
    ]
...
def plot_sentiment_hist(counts: pd.Series, output_dir: Path) -> None:
    ...
    sns.barplot(
        x=counts.index, y=counts.values, hue=counts.index, palette="Set2", legend=False
    )
    plt.title("文本分类标签分布")
    plt.xlabel("情感标签")
    plt.ylabel("样本数量")
...
def plot_bio_heatmap(matrix: pd.DataFrame, output_dir: Path) -> None:
    ...
    sns.heatmap(
        matrix, annot=True, fmt="d", cmap="YlOrRd", cbar_kws={"label": "标签数量"}
    )
