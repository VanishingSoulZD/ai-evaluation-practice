Terminal-Bench, ProgramBench, Online-Mind2Web, ARC Prize, ARC-AGI leaderboard

OmniDocBench, SQuAD, Memory Sources

序号 核心实体 / 术语 所属方向 类别 应用场景与重要性说明 重要性评分
1 SWE-Bench 自动化评测 Benchmark 核心代码工程评测：评估LLM解决真实世界复杂软件工程问题能力的绝对标准。理解单元测试验证机制，掌握作为自动化评测基准的操作。 10
2 SWE-Bench Pro 自动化评测 Benchmark 高难度代码修复与多步推理评测，用于量化Agent在复杂任务中的能力。 10
3 ARC-AGI-3 自动化评测 Benchmark 前沿推理能力评估：测试大模型在未知环境中的自主探索与适应能力，为强推理Agent评测设计提供参考。 10
4 SaaS-Bench 自动化评测 Benchmark 真实多应用工作流评测，覆盖企业办公场景的长程任务，关键指标用于衡量Agent实际办公能力。 9
5 DeepSWE 评测/标注结合 Benchmark / 数据构建 高质量数据与评测集构建：零污染、验证器、原创任务生成，用于训练和验证评测环境。 9
6 GDPval-AA 自动化评测 Benchmark 面向真实业务场景的评测，考核Agent长期决策能力和环境交互能力。 9
7 Terminal-Bench Hard 自动化评测 Benchmark 高复杂度终端操作与连续任务评测，直接衡量Agent在命令行及自动化操作中的表现。 9
8 GPQA Diamond 自动化评测 Benchmark 硬核推理评测集，用于衡量高级Agent的多领域知识推理能力。 9
31 GPQA 自动化评测 Benchmark 高难度推理数据集，作为评测底座，用于衡量Agent推理能力。 9
32 MMMU-Pro 自动化评测 Benchmark 多模态任务评测集，用于量化复杂输入下的Agent表现。 9

排名 排行榜 官方网站
1 SWE-bench Leaderboard (SWE-bench)
2 ARC Prize / ARC-AGI Leaderboard (ARC Prize)
3 GDPval Leaderboard (OpenAI Evals)
4 Terminal-Bench Leaderboard (Terminal-Bench)
5 LiveCodeBench Leaderboard (LiveCodeBench)
6 Chatbot Arena Leaderboard (Arena AI)
7 MMMU Leaderboard (MMMU Benchmark)
实体 官网 / 官方出处
SWE-Bench (SWE-bench)
SWE-Bench Pro (SWE-bench)
ARC-AGI-3 (ARC Prize)
SaaS-Bench (UniPat AI)
DeepSWE (DeepSWE)
GDPval-AA (OpenAI)
Terminal-Bench Hard (Terminal-Bench)
GPQA Diamond (Paper与代码)
GPQA (Paper与代码)
MMMU-Pro (MMMU Benchmark)

https://www.anthropic.com/news/claude-fable-5-mythos-5

Agentic coding
SWE-Bench Pro
Agentic coding
FrontierCode (Diamond)
Knowledge work
GDPval-AA
Knowledge work vision
GDP.pdf
Spatial reasoning
Blueprint-Bench 2
Tool use
AutomationBench
Computer use
OSWorld-Verified
Legal
Legal Agent Benchmark
Multidisciplinary reasoning
Humanity's Last Exam
Biology
BioMysteryBench
Agentic coding
Terminal-Bench 2.1
Cybersecurity
ExploitBench (Cap%)
Health
HealthBench Professional

https://openai.com/zh-Hans-CN/index/gpt-5-6/
专业能力
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Fable 5 Claude Opus 4.8 Gemini 3.1 Pro 预览版 Gemini 3.5 Flash
Agents' Last Exam 52.7% 50.4% 50.3% 46.9% 40.5% 45.2% 32.1% —
GDPval-AA v2 1,747.8 Elo 1,593 Elo 1,591.8 Elo 1,493.7 Elo 1,759.6 Elo 1,600.1 Elo 962.3 Elo 1,348.8 Elo
管理咨询任务 (内部) 43.2% 37.2% 35.4% 31.3% 35.5% 31.6% 13.2% —
Big Finance Bench 53% 51% 36% 49% — 44% — —
Artificial Analysis Intelligence Index v4.1 58.9 Index 得分 55 Index 得分 51.2 Index 得分 54.8 Index 得分 59.9 Index 得分 55.7 Index 得分 46.5 Index 得分 50.2 Index 得分
编码
评测 GPT‑5.6 Sol GPT‑5.6 Sol Ultra GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Fable 5 Claude Opus 4.8 Gemini 3.1 Pro 预览版
Artificial Analysis Coding Agent Index v1.1 80 Index 得分 — 77.4 Index 得分 74.6 Index 得分 76.4 Index 得分 — — 77.2 Index 得分 72.5 Index 得分 42.7 Index 得分
SWE-Bench Pro 64.6% — 63.4% 62.7% 59.4% 80.3% 77.8% 80% 69.2% 54.2%
DeepSWE v1.1 72.7% — 69.6% 67.2% 67% — — 69.7% 59% 11.8%
Terminal-Bench 2.1 88.8% 91.9% 87.4% 84.7% 85.6% 88% — 83.1% 78.9% 70.7%
安全
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 GPT‑5.4 Claude Opus 4.8 Claude Mythos 5 Claude Mythos 预览版
Healthbench Professional 60.5% 57.7% 55.7% 51.8% 48.1% 52.6% 66% 64.7%
计算机使用
评测 GPT‑5.6 Sol GPT‑5.6 Sol Ultra GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Opus 4.8 Gemini 3.1 Pro 预览版
OSWorld 2.0 62.6% — 50.2% 45.6% 47.5% — — 54.8% —
BrowseComp 90.4% 92.2% 87.5% 83.3% 84.4% 88% 87.9% 84.3% 85.9%
BenchCAD 70.6% — 62.3% 63.1% 44.4% 38.4% 35.5% 27.3% —
BenchCAD (python 工具) 83.4% — 78.2% 73.9% 55.8% 65% 61% 51.8% —
网络安全
评测 GPT‑5.6 Sol GPT‑5.6 Sol Ultra GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Opus 4.8
夺旗挑战 96.7% — 91.8% 85.2% 88.1% — — —
SEC-Bench Pro 71.2% 74.3% 57.7% 48.9% 45.8% — — —
ExploitBench 73.5% — 52.9% 33.2% 47.9% 78% 74.2% 40%
ExploitGym 33.7% — 23.2% 12.4% 15.1% — — —
自我优化
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5
Internal Research Debugging Evaluation 68.3% 67.8% 50.8% 50%
KernelGen 1P 61.1% 49.2% 22.4% 29.3%
NanoGPT 9.69% 14.5% 1.66% 2.65%
PostTrainBench Lite 50.3% 51.5% 29.6% 38.8%
RSI Index 57.9% 56.3% 41.9% 41.7%
多模态
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Fable 5 Claude Opus 4.8 Gemini 3.1 Pro 预览版
MMMU Pro (无工具) 83% 80.7% 78.4% 81.2% — — 80.5%
MMMU Pro (使用工具) 84.6% 82% 79.5% 83.2% — — —
gdp.pdf 30.7% 24.7% 22.7% 26% 29.8% 22.5% 16.7%
学术
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Fable 5 Claude Opus 4.8 Gemini 3.1 Pro 预览版
GPQA Diamond 94.6% 92.9% 92.3% 93.6% 94.1% 94.6% 92.6% 92% 94.3%
FrontierMath Tier 1-3 (v2) 89% 84.9% 78.6% 85.3% — — 87% 80% 59.6%
FrontierMath Tier 4 (v2) 83% 68.3% 58.5% 72.5% — — 87.8% 56.1% —
工具使用
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Fable 5 Claude Opus 4.8 Gemini 3.1 Pro 预览版 Gemini 3.5 Flash
AutomationBench 18.1% 15.2% 14.9% 12.9% — — 17.4% 15.5% — 14.5%
Toolathlon 58% 53.1% 53.4% 55.6% 61.7% 61.1% 61.7% 59.9% 48.8% —
长上下文
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Mythos 5 Claude Mythos 预览版 Claude Opus 4.8
OpenAI MRCR v2 8-needle 256K-512K 91.5% 89.6% 41.3% 81.5% — — —
OpenAI MRCR v2 8-needle 512K-1M 73.8% 72.5% 41.3% 74% — — —
GraphWalks BFS 256k f1 90.7% 76.9% 81.3% 73.7% 91.1% 85.7% 85.9%
GraphWalks BFS 1mil f1 77.1% 71.2% 51.2% 45.4% 79.4% 74.3% 68.1%
抽象推理
评测 GPT‑5.6 Sol GPT‑5.6 Terra GPT‑5.6 Luna GPT‑5.5 Claude Opus 4.8 Gemini 3.1 Pro 预览版
ARC-AGI-3⁷ 7.78% 0.8% 0.18% 0.43% 1.5% 0.42%

https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro
Benchmark (Metric) # Shots DeepSeek-V3.2-Base DeepSeek-V4-Flash-Base DeepSeek-V4-Pro-Base
Architecture - MoE MoE MoE

# Activated Params - 37B 13B 49B

# Total Params - 671B 284B 1.6T

World Knowledge
AGIEval (EM) 0-shot 80.1 82.6 83.1
MMLU (EM) 5-shot 87.8 88.7 90.1
MMLU-Redux (EM) 5-shot 87.5 89.4 90.8
MMLU-Pro (EM) 5-shot 65.5 68.3 73.5
MMMLU (EM) 5-shot 87.9 88.8 90.3
C-Eval (EM) 5-shot 90.4 92.1 93.1
CMMLU (EM) 5-shot 88.9 90.4 90.8
MultiLoKo (EM) 5-shot 38.7 42.2 51.1
Simple-QA verified (EM) 25-shot 28.3 30.1 55.2
SuperGPQA (EM) 5-shot 45.0 46.5 53.9
FACTS Parametric (EM) 25-shot 27.1 33.9 62.6
TriviaQA (EM) 5-shot 83.3 82.8 85.6
Language & Reasoning
BBH (EM) 3-shot 87.6 86.9 87.5
DROP (F1) 1-shot 88.2 88.6 88.7
HellaSwag (EM) 0-shot 86.4 85.7 88.0
WinoGrande (EM) 0-shot 78.9 79.5 81.5
CLUEWSC (EM) 5-shot 83.5 82.2 85.2
Code & Math
BigCodeBench (Pass@1) 3-shot 63.9 56.8 59.2
HumanEval (Pass@1) 0-shot 62.8 69.5 76.8
GSM8K (EM) 8-shot 91.1 90.8 92.6
MATH (EM) 4-shot 60.5 57.4 64.5
MGSM (EM) 8-shot 81.3 85.7 84.4
CMath (EM) 3-shot 92.6 93.6 90.9
Long Context
LongBench-V2 (EM) 1-shot 40.2 44.7 51.5

https://qwen.ai/blog?id=qwen3.7
Opus-4.6 Max K2.6 Thinking GLM-5.1 Thinking DS-V4-Pro Max Qwen3.6-Plus Qwen3.7-Max
Coding Agent
Terminal Bench 2.0-Terminus 65.4 66.7 63.5 67.9 61.6 69.7
SWE-Verified 80.8 80.2 -- 80.6 78.8 80.4
SWE-Pro 57.3 59.5 58.8 59.0 56.6 60.6
SWE-Multilingual 77.5 76.7 -- 76.2 73.8 78.3
NL2repo 47.6 42.8 41.0 35.5 34.4 47.2
SciCode 51.9 52.2 45.1 -- 41.4 53.5
QwenWebDev 1617 -- 1564 1570 1500 1568
QwenSVG 1541 1325 1605 1506 1432 1608
General Agent
Qwenclaw 65.5 54.7 58.7 59.2 57.2 64.3
CoWorkBench 68.2 58.2 66.0 66.3 64.5 67.2
ClawEval 70.4 61.5 62.7 58.4 57.1 65.2
Skillsbench -- 56.2 53.1 52.3 45.7 59.2
BFCL-V4 76.7 71.3 70.9 70.6 68.9 75.0
MCP-Mark 56.7 55.9 57.5 57.1 48.2 60.8
MCP-Atlas 75.8 66.6 71.8 73.6 74.1 76.4
Vitabench -- 39.1 45.1 51.9 42.8 47.9
SpreadSheetBench-v1 89.3 84.5 85.2 84.9 80.2 87.0
Kernel Bench L3 2.63/98% 1.41/80% 2.00/78% 1.07/54% 1.03/48% 1.98/96%
HLE w/ tools 53.0 54.0 52.3 48.2 50.2 53.5
QwenWorldBench 56.1 50.9 50.2 52.3 47.6 57.3
STEM & Reasoning
GPQA Diamond 91.3 90.5 86.2 90.1 90.4 92.4
HLE 40.0 36.4 34.7 37.7 28.8 41.4
LiveCodeBench 88.8 89.6 -- 93.5 87.1 91.6
HMMT 2026 Feb 96.2 92.7 89.4 95.2 87.8 97.1
IMOAnswerBench 75.3 86.0 83.8 89.8 83.8 90.0
CritPT 12.6 8.0 4.6 12.9 2.9 11.4
Apex 34.5 24.0 11.5 38.3 8.8 44.5
General Capability
MMLU-Pro 89.7 87.1 86.3 87.5 88.5 89.6
MMLU-Redux 95.2 95.3 94.3 94.8 94.5 95.0
SuperGPQA 72.5 71.3 68.0 69.9 71.6 73.6
IFEval 91.9 94.5 94.5 91.9 94.3 94.3
IFBench 62.5 76.0 76.0 77.0 74.2 79.1
MRCR-v2 128k 84.0 63.1 62.0 74.4 85.9 90.4
Multilingualism
WMT24++ 82.7 81.6 81.8 82.2 84.3 85.8
MAXIFE 81.3 87.7 87.7 88.9 88.2 89.2
MMMLU 90.6 87.5 87.2 87.9 89.5 90.3
MMLU-ProX 86.1 83.7 83.9 83.9 84.7 87.0
NOVA-63 59.1 56.7 54.6 52.8 57.9 59.0
INCLUDE 87.4 84.2 84.3 86.1 85.1 86.2
Global PIQA 91.2 89.2 89.5 90.5 89.8 91.4
PolyMATH 80.2 82.7 67.6 72.0 77.4 86.5
